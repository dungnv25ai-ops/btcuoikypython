...
class Boss10LogicMixin:
    ...
    def _b10_chem_kiem(self):
        ...
        dx = px - bx
        dy = py - by
        self._ds_kiem_bay.add(KiemBay(bx, by, dx, dy, phase=self._b10_phase))
    ...
    def _b10_update_sk4(self):
        """Spawn từng kiếm mưa theo interval."""
        if not (self._b10_sk4_active
                and self._b10_sk4_spawned < self._b10_sk4_sokem):
            return
        self._b10_sk4_dem += 1
                ...
                if self._b10_enrage_seq == 0:
                    ...
                    if b:
                        ...
                        if (vung7.colliderect(self.nhan_vat.rect)
                            ...
                            self._b10_phan_cong()
                    self._b10_enrage_seq  = 0
                    self._b10_enrage_timer = 1 * FPS

        # ── Bị động phase2 thường (không enrage) ──────────
        elif self._b10_phase == 2 and not self._b10_enrage:
