...
class Boss5(pygame.sprite.Sprite):
    ...
    def __init__(self, cot, hang):
        ...
        self._flip = False   # False = ảnh gốc (nhìn trái), True = lật (nhìn phải)
    ...
    def _anh_goc(self):
        """Surface gốc theo hướng hiện tại (đã flip nếu cần)."""
        return self._surf_flip if self._flip else self._surf

    def quay_ve(self, player_x):
        """Quay mặt boss về phía player. Ảnh gốc mặc định nhìn TRÁI.
        Player ở bên phải boss → lật ảnh (nhìn phải); bên trái → giữ nguyên."""
...
class Boss10(pygame.sprite.Sprite):
    ...
    def __init__(self, cot, hang):
        ...
        self._tu_luc = 0
        self.can_ban = False
        self.mau     = self.SO_MAU_MAX   # HP hiện tại
        self._flash  = 0                 # frame nháy trắng khi bị đánh
